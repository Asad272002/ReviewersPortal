...
export async function GET(request: NextRequest, ctx: { params: Promise<{ deliverableId: string }> }) {
  ...
  if (!Number.isFinite(id) || id <= 0) return NextResponse.json({ success: false, message: 'Invalid deliverable id' }, { status: 400 });

  try {
    const client = createDeepOpsClient({ mode: 'live' });
    const deliverable = await client.getDeliverableById(id);
    return NextResponse.json({ success: true, data: deliverable }, { status: 200 });
  } catch (err: any) {
