...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const gradingMode = body?.gradingMode != null ? String(body.gradingMode) : undefined;
    ...
    if (gradingMode !== undefined) patch.grading_mode = gradingMode;
    ...
    if (questions) {
      ...
      const rows = questions.map((q: any, idx: number) => {
        ...
        if (type === 'mcq') {
          ...
          base.correct_answers = q?.correctAnswers ?? [];
        } else {
          // text/scenario question: allow optional answerKey via options object
          base.options = q?.options ?? null;
          base.correct_answers = null;
        }
        return base;
      });
      if (rows.length > 0) {
