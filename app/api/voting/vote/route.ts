...
export async function POST(request: NextRequest) {
  try {
    ...
    const voteRows = await supabaseService.getVotes();
    const proposalRows = await supabaseService.getProposals();
    ...
    await supabaseService.addVote(proposalId, String(userId), username, voteType);

    // Recompute aggregated voting results for this proposal and persist
    try {
      const freshVotes = await supabaseService.getVotes();
    ...
    const voterCount = voterKeys.has('') ? voterKeys.size - 1 : voterKeys.size;
      ...
      await supabaseService.updateVotingResult(proposalId, {
